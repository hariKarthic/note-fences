export class FENCE {
    ...
    isInside(check) {
        ...
        // const TEMP = []
        const ENTERED_FENCES = [];
        for (const point in dataPoints) {
            const isFound = this.pointInsideCircle(check, { center: dataPoints[point].center, radius: dataPoints[point].radius });
            if (isFound) {
                ENTERED_FENCES.push(dataPoints[point])
            }
            //TEMP.push(isFound)
        }
        //const LOOK = TEMP.includes(true);
        //return LOOK;
        return ENTERED_FENCES;
    }
}